...
class PathResolverNode(AutoNode):
    ...
    def onResolve(self):
        ...
        while lookahead is None and radius <= 4:
            lookahead = self.m_purePursuit.get_lookahead_point(current[0], current[1], radius)
            radius *= 1.2
            
        draw_pp(current, lookahead, self.m_purePursuit.path)

        motorMsg = MotorInput()
        motorMsg.left_motor = 0.0
        motorMsg.right_motor = 0.0
        ...
        sortaDist = (lookahead[1] - current[1]) ** 2 + (lookahead[0] - current[0]) ** 2
        if self.m_backCount == -1 and (lookahead is not None and sortaDist > 0.1):
            wanted_heading = math.atan2(lookahead[1] - current[1], lookahead[0] - current[0])
            error = self.getAngleDifference(wanted_heading, self.m_position.theta) / math.pi
